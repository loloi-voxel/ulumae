...
ALTER TABLE users 
ADD COLUMN IF NOT EXISTS dead_mans_switch_delay_months INTEGER DEFAULT 12
    CHECK (dead_mans_switch_delay_months IN (3, 6, 12, 24));

ALTER TABLE users ADD COLUMN IF NOT EXISTS dead_mans_switch_warning_30_sent_at TIMESTAMPTZ;
ALTER TABLE users ADD COLUMN IF NOT EXISTS dead_mans_switch_warning_7_sent_at TIMESTAMPTZ;
ALTER TABLE users ADD COLUMN IF NOT EXISTS dead_mans_switch_warning_1_sent_at TIMESTAMPTZ;
ALTER TABLE users ADD COLUMN IF NOT EXISTS dead_mans_switch_transferred_at TIMESTAMPTZ;
...
-- 1. Add arweave_url to memorial_media_assets (what's causing your current crash)
ALTER TABLE memorial_media_assets
ADD COLUMN IF NOT EXISTS arweave_url TEXT;

-- 2. Add arweave_tx_id to memorial_media_assets (likely also queried)
ALTER TABLE memorial_media_assets
ADD COLUMN IF NOT EXISTS arweave_tx_id TEXT;

-- 3. Add expires_at to user_session_devices (used in session tracking)
ALTER TABLE user_session_devices
ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ;

-- 4. Add inviter_email to witness_invitations (used in invite route)
...
ADD COLUMN IF NOT EXISTS seal_status TEXT 
  CHECK (seal_status IN ('pending', 'in_progress', 'completed', 'failed')),
ADD COLUMN IF NOT EXISTS seal_job_id TEXT,
ADD COLUMN IF NOT EXISTS seal_selected_asset_ids JSONB DEFAULT '[]';
